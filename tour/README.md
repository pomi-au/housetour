...
6. **Mapped mode.** `L`, or the checkbox in the `T` panel, switches the house lights to baked light maps (`bakeLightMaps` in `model/lighting.js`): a ceiling map and a floor map over the footprint at 4 cm, one map per wall piece at 6 cm, and one texel per fixture part, all HDR float textures with the same cone, halo, falloff and room mask as the run-time lights. The run-time spot and point lights leave the scene, so every shader drops its light loops. Baking takes about a second on the 10024 sample. Baked light is diffuse only: specular highlights from the downlights show in real-time mode only.
