...
| `tour/js/fixtures/skeleton.js` | `createSkeleton()` is the tour's procedural skeleton; `popIn(group, { from, to, scale, yaw })` animates it in. The viewer keeps one: a hinged door that opens into a confined space (a raster region under 1.5 m² whose volume is 1 m³ or less, `PlanTour.setConfinedVolume(m3)` changes the limit) makes it pop in behind the door, scaled to fit, facing the walker; it leaves when that door closes. |
