...
The page opens as a plan view and turns over into the isometric view. Drag to orbit, scroll to zoom, right-drag to pan. Click a floor to walk on it (the click looks through the roof and walls for the slab). Doors and sliding doors open on click, `E` or a tap, with the tour's lime outline on the one under the crosshair; a door swings away from the walker. `T`, or the gear button, opens the settings panel; the tour waits while it is open and continues when it is closed. On a touch device the walk has an analog stick bottom left (deflection sets direction and speed, the outer rim runs, and the head turns to face the direction of travel), a hand button bottom right that uses the door under the crosshair, and the settings and exit buttons top right. One finger elsewhere turns the view: the turn continues at a rate set by how far the finger sits from where it touched, until it lifts. Two fingers pinch to zoom.

The walker carries a lantern, a warm point light that comes on while moving and fades out when still; it uses a spare mask slot, so it lights whatever room it is in. `P`, the plane button in the top bar, or the panel checkbox turns the walker into a paper plane: a small folded dart seen from behind with a 95° field of view. It cruises forward on its own, W or the stick forward speeds it up, S slows it to a hover, A and D or the stick sideways steer, the look controls pitch and yaw, and it banks into turns. It bumps off walls and closed doors, keeps off the floor and ceiling, and stays on the lot. `P` again lands it where it is. The walking controls, lighting panel (`T`), daylight clock, floor reflection, procedural textures, lawn, streets, lamps and lot boundary are the tour's. There is no neighbourhood. The model is centred on the lawn; the lot, hedge and street pitch grow with it.
...
| `tour/js/fixtures/sliding-door.js` | `createSlidingDoor({ width, height, depth, position, rotationDeg, materials, slide, panel })`: frame and panels, one slides on `toggle()`. `panel: 'glass'` is the patio slider; `panel: 'mirror'` is the robe slider, solid painted leaves with a glossy 25 % planar mirror on the room face (a Reflector the viewer renders itself, one bounce, near and on screen only). The viewer picks mirror for sliders labelled INTERNAL or with a room or wall on both sides 0.9 m out and a width under 3 m; stackers stay glazed. A robe is a confined space, so the skeleton rule applies to it. |
| `tour/js/fixtures/skeleton.js` | `createSkeleton()` is the tour's procedural skeleton; `popIn(group, { from, to, scale, yaw })` animates it in. The viewer keeps one: a hinged door that opens into a confined space (a raster region under 1.5 m² whose volume is 8 m³ or less, `PlanTour.setConfinedVolume(m3)` changes the limit) makes it pop in behind the door, scaled to fit, facing the walker; it leaves when that door closes. |
| `tour/js/fixtures/paper-plane.js` | `createPaperPlane({ span, length })`: a folded paper dart for the plane mode; `userData.bank(angle)` rolls it. |
...
6. **Baked mode (default).** The walk renders the house lights from baked light maps (`bakeLightMaps` in `model/lighting.js`): a ceiling map and a floor map over the footprint at 4 cm, one map per wall piece at 6 cm, and one texel per fixture part, all HDR float textures. Direct light uses the same cone, halo, falloff and room mask as the run-time lights. Bounce light is a radiosity solve: the surfaces become patches (0.8 m on floors and ceilings, 1 m on walls, both sides), each patch's radiosity is its albedo times the light it receives, three bounces pass between patches that share a room, and a final gather onto a 0.25 m receiver grid per surface is interpolated onto the fine texels. Albedo comes from each material's colour and metalness. The run-time spot and point lights leave the scene, so every shader drops its light loops. The bake takes about 0.6 s on the 10024 sample and 2 s on carabeen16. `L`, or the checkbox in the `T` panel, switches to the real-time pool for comparison; that mode has specular highlights from the downlights but no bounce light. A room that cannot fit a light with wall clearance still gets one at the centre of its largest rectangle.
