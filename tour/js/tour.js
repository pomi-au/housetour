...
float roomMask(int i, vec3 vRoomPos) {
  ...
  // Small helpers shared by the skeleton builders: a mesh at a point, a bone between two joints, a joint ball.
  ...
  // Skeleton taking a bath: origin on the tub floor at its centre, +z toward the taps (feet), -z toward the head end.
  // Reclined against the head end, knees up, arms out on the rims, skull above the rim looking down the tub.
  function buildBathingSkeleton(halfWidth = 0.34) {
    const g = new THREE.Group();
    const bone = boneMaterial();
    const { add, V, link, joint } = boneKit(g, bone);
    const pelvis = V(0, 0.10, 0.12);
    const up = V(0, Math.sin(0.95), -Math.cos(0.95)).normalize();   // torso leans back about 55 degrees from vertical
    // Pelvis and spine.
    const pel = add(new THREE.TorusGeometry(0.12, 0.022, 8, 24), pelvis.x, pelvis.y, pelvis.z); pel.rotation.x = Math.PI / 2; pel.scale.z = 0.6;
    const spineTop = pelvis.clone().addScaledVector(up, 0.52);
    for (let i = 0; i < 18; i++) {
      const p = pelvis.clone().addScaledVector(up, 0.03 + i * 0.028);
      const v = add(new THREE.CylinderGeometry(0.022, 0.022, 0.02, 8), p.x, p.y, p.z);
      v.quaternion.setFromUnitVectors(V(0, 1, 0), up);
    }
    // Rib cage rings around the upper spine, sternum in front, clavicles at the top.
    const front = V(0, up.z, -up.y).negate();   // perpendicular to the spine, toward the chest
    ...
    ribs.forEach((r, i) => {
      ...
      const ring = add(new THREE.TorusGeometry(r, 0.007, 6, 28), p.x, p.y, p.z);
      ring.quaternion.setFromUnitVectors(V(0, 0, 1), up); ring.scale.y = 0.7;
    });
    const st = pelvis.clone().addScaledVector(up, 0.33).addScaledVector(front, 0.125);
    const sternum = add(new THREE.BoxGeometry(0.03, 0.26, 0.015), st.x, st.y, st.z); sternum.quaternion.setFromUnitVectors(V(0, 1, 0), up);
    const neck = spineTop.clone();
    link(neck, neck.clone().add(V(-0.18, -0.02, 0)), 0.008); link(neck, neck.clone().add(V(0.18, -0.02, 0)), 0.008);
    // Skull above the rim, tilted a little back, looking down the tub toward the feet.
    const skull = buildSkull(bone);
    skull.position.copy(spineTop.clone().addScaledVector(up, 0.10));
    skull.rotation.x = -0.35;
    g.add(skull);
    // Arms out along the rims.
    for (const sgn of [-1, 1]) {
      ...
      link(el, wr, 0.011); link(el.clone().add(V(sgn * 0.01, 0.01, 0)), wr.clone().add(V(sgn * 0.006, 0.008, 0)), 0.008);
      const hand = add(new THREE.BoxGeometry(0.055, 0.016, 0.08), wr.x, wr.y, wr.z + 0.05);
      for (let f = 0; f < 4; f++) { const fg = add(new THREE.CylinderGeometry(0.005, 0.004, 0.07, 6), wr.x - 0.02 + f * 0.013, wr.y, wr.z + 0.125); fg.rotation.x = Math.PI / 2; }
      void hand;
      // Legs: knees up, feet flat toward the taps.
      const hip = V(sgn * 0.09, 0.11, 0.15), kn = V(sgn * 0.10, 0.34, 0.52), an = V(sgn * 0.09, 0.05, 0.92);
      joint(hip, 0.03); link(hip, kn, 0.02); joint(kn, 0.032);
      link(kn, an, 0.016); link(kn.clone().add(V(sgn * 0.02, 0, 0)), an.clone().add(V(sgn * 0.015, 0, 0)), 0.008);
      joint(an, 0.022); add(new THREE.BoxGeometry(0.075, 0.04, 0.2), an.x, 0.035, an.z + 0.06);
    }
    return g;
  }
  ...
  function buildWalkScene() {
    ...
    let tubSkeleton = null;
    {
      ...
      if (shell && door) {
        ...
        const k = Math.min(1, (len - 0.15) / 1.35);   // the pose spans about 1.35 m from head end to toes
